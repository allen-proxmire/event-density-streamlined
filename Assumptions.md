...
| P05 | Polarity is transported between neighbouring loci. | Transport is a hop to the next locus: forward (A) or backward (B). |
...
| | assumption |
|---|---|
| M1 | Transport for a family of N channels along a chain is H(k) = e^{ik}A + e^{−ik}B, with A the forward hop and B the backward hop. |
| M2 | Reflection sends k → −k and reverses the order of the channels (S: channel i ↦ channel N+1−i). A reflection-symmetric transport therefore satisfies S H(k) S⁻¹ = H(−k). |
