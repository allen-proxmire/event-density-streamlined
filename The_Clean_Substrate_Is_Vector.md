...
**Series:** Event Density (ED) papers, matter-sector chirality. Successor to *The Parity Wall*, which it supersedes on the verdict's tier.
...
**Version:** v2 — 2026-09-12 — outside review note added, claims downgraded

---

> **Review note (2026-09-12).** The text below is unchanged apart from this note and the removal of references to files outside this repository. These corrections come from the outside review of 2026-09-12 and take precedence over the text.
>
...
This was checked numerically as a control on the proof (`tools/check_result.py`): for `N = 1` through `6`, over many random forward hops `A`, the parity-clean winding is `0` in every case, matching the theorem.
...
- `tools/check_result.py` — the winding computation (clean-case theorem control for `N = 1..6`; broken-case `winding = N`).
